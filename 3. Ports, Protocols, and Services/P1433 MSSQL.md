...
**SQL Server Agent:** Used for scheduling and automating tasks.
**SQL Server Reporting Services (SSRS):** Used for generating and managing reports.
...
| authentication_type                 | Int           | Applies to: SQL Server 2012 (11.x) and later. Signifies authentication type. The following are the possible values and their descriptions. 0 : No authentication 1 : Instance authentication 2 : Database authentication 3 : Windows authentication 4 : Azure Active Directory authentication                                       |
...
- **Get specific user:**
	...
	SELECT user_name();
	```
- **Get current database users (not the server):** Useful when you cannot access the table sys.database_principals
	```sql
	EXEC sp_helpuser
...
- MSSQL could allow you to execute scripts in Python and/or R. These will be executed by a different user than the one using xp_cmdshell to execute commands.
	```c
...
3. **Identify service account using `xp_cmdshell`:**
	...
	2> GO
	```
4. **Bypass blacklisted "EXEC xp_cmdshell":**
	```c
